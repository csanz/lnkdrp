...
### Public share
- **M8. Locked data room answers 404 vs 401 per candidate docId on `download-requests`.** `src/app/api/share/[shareId]/download-requests/route.ts:104-128`. `resolveProjectStatsTarget` runs before `shareLinkUnlocked`; SECURITY.md §7.8's "third door" oracle, fixed on stats and pages but not here. Fix: resolve link → unlock check → then `findProjectDocument`. **Closed 2026-09-25:** the route resolves the link, checks the unlock, and only then looks up the document; a locked room answers 401 for every docId. `tests/lib/downloadRequestLockedRoom.test.ts`; SECURITY.md 7.8 updated.

### Admin / cron
- **M9. Nightly `analytics-reconcile` does three full-collection scans in one 300 s function.** `src/app/api/cron/analytics-reconcile/route.ts:76-103`, `src/lib/analytics/reconcileLinkCounters.ts:75-91, 116-118`. Range on `lastViewedAt`/`lastEventAt` alone cannot use the compound indexes (leading `docId`/`shareId`); plus unfiltered `$group` over `shareviews` and `find({})` over `sharelinks`. No lease. Fix: iterate per org or add leading-field indexes; bound with a lease. **Closed 2026-09-25:** the job takes the cron lease, `db/migration/20260925_0002_analytics_activity_indexes.mjs` adds single-field `lastViewedAt` / `lastEventAt` indexes, and the reconcile is bound to links active since `?days=2` (`?full=1` for a full pass). Explain on the dev database: every scan moved from COLLSCAN to IXSCAN. `tests/lib/analyticsReconcileWindow.test.ts`.
...
- **M13. `/api/admin/shareviews/doc/:docId` is unbounded.** `src/app/api/admin/shareviews/doc/[docId]/route.ts:38-83`. Fix: `?limit` (cap 500) + cursor. **Closed 2026-09-25:** `?limit` (default 100, cap 500) and a `(updatedDate, _id)` keyset cursor (`src/lib/http/dateIdCursor.ts`); the admin page loads 200 and has a "Load older views" button.

### MCP / AI
- **M14. Destructive-tool confirmation degrades to the agent's own `confirm: true` on headless/cancelling clients; docs/MCP.md claims the opposite.** `mcp/src/confirm.ts:182, 195-203, 221`, `docs/MCP.md:24`. Fix: pick one policy; at minimum refuse a pre-set `confirm: true` on the call that produced the cancel; fix the doc. **Closed 2026-09-25:** on an elicitation-capable client the prompt is always tried first; a dismissed or undelivered prompt refuses the call with the preview whatever `confirm` says, and only the next call for that action within 10 minutes may carry `confirm: true`. A decline is final. docs/MCP.md says exactly this. `tests/lib/mcpConfirm.test.ts`.
- **M15. Document titles reach the agent unwrapped in the confirmation error message.** `mcp/src/confirm.ts:149-155, 163, 224` and tool callers. Titles are model-derived from PDF text; every read tool wraps them, this path does not, on exactly the path where `confirm: true` is the gate. Fix: `sanitizeUntrustedText` on headline/facts. **Closed 2026-09-25:** headline and facts are sanitised before they reach the elicitation message, the refusal text and `details.preview`; `details.previewNote` carries the standard note.
- **M16. Inline uploads let one write key exhaust memory: 66 MB base64 accepted, Ghostscript + pdfjs unbounded.** `mcp/src/tools/sharePdf.ts:139-146, 230, 243`, `mcp/src/optimize.ts:276-297`. The app rejects >4.5 MB anyway. Fix: cap at the platform ceiling; semaphore Ghostscript/pdfjs. **Closed 2026-09-25:** inline input is refused above 16 MB from the base64 length before decoding; what is sent after optimisation is capped at 3 MB against a non-localhost API (`too_large` names the size and points at `sourceUrl`); Express body limit on `/mcp` is 24 MB with a JSON-RPC 413; Ghostscript and pdfjs run under a semaphore of 2. `tests/lib/mcpSemaphoreAndInlineLimits.test.ts`.
- **M17. No cap on MCP sessions per key or in total.** `mcp/src/main.ts:164-232, 342-349`. One key looping `initialize` at 300/min → ~18k live sessions in the idle window. Fix: per-key and global caps. **Closed 2026-09-25:** `LNKDRP_MCP_MAX_SESSIONS_PER_KEY` (20, stalest session of that credential closed) and `LNKDRP_MCP_MAX_SESSIONS` (500, `initialize` answers JSON-RPC 429), keyed on the credential id so API keys and OAuth grants count alike; reported by `/healthz`. Verified live with a cap of 3. `tests/lib/mcpSessionCaps.test.ts`.

### Infra / models
- **M18. Catch-alls map infrastructure failures to HTTP 400 and skip ErrorEvent logging.** `src/lib/http/errorResponse.ts:77-88, 99-104` and 12 routes (`docs`, `sidebar`, `share/stats`, `unlock`, `landing`…). `errorJson` logs only for `status >= 500`. Fix: use 500 in catch-alls, or log whenever `code === UNHANDLED_EXCEPTION`. **Closed 2026-09-25:** `errorJson` logs every response it builds, and the twelve catch-alls answer 500; validation 400s untouched.
- **M19. `ErrorEvent.stack` stored unredacted.** `src/lib/errors/logger.ts:410-446`, `serializeErrorEvent.ts:58`. `Error.stack` begins with the raw message, so emails/URIs redacted from `message` survive in `stack` and render in `/a`. Fix: `redactLogText` on both. **Closed 2026-09-25:** the stack is redacted at write time and again in the admin serializer for rows written before. `tests/lib/errorEventStackRedaction.test.ts`.
...
### Frontend
- **M21. `Markdown` renders every inline code span as a block and nests `<pre>` in `<pre>`.** `src/components/Markdown.tsx:177-215` branches on `inline`, removed in react-markdown v9. `HelpMarkdown.tsx:55-75` does it right. Fix: detect by `language-` class; override `pre`. **Closed 2026-09-25:** `src/lib/client/markdownCode.ts` (language class or a newline means a block) shared by `Markdown` and `HelpMarkdown`; `pre` is overridden so a block owns one `<pre>`. `tests/lib/markdownCode.test.ts` renders both cases.
...
- **M25. Client-side first-page thumbnail silently never renders.** `src/lib/client/docUploadPipeline.ts:74-80`, `doc/update/[code]/pageClient.tsx:74-80`, `r/[token]/pageClient.tsx:68-72`. `disableWorker` does not exist in pdf.js 5 and no `workerSrc` is set; the fallback rejects and the catch returns null. Fix: set `GlobalWorkerOptions.workerSrc` or reuse the shared `workerPort`. **Closed 2026-09-25:** one `renderPdfFirstPagePngBestEffort` in `src/lib/client/pdfThumbnail.ts` replaces the three copies and shares the viewer's worker. Verified in headless Chrome on a replace link: the client rendered and uploaded `preview.png` and the PATCH carried `previewImageUrl`.
- **M26. History list races on sort/page-size change; stale cursor then paginates the wrong query.** `history/pageClient.tsx:416-442, 499-513`. Fix: pass the cancel token into `refreshFirstPage`. **Closed 2026-09-25:** a query generation counter; responses from a superseded query, and pages whose cursor came from one, are dropped.
- **M27. Project settings: guide-upload errors swallowed after the modal closes.** `project/[projectSlug]/pageClient.tsx:758-855`. Review enabled with no guide. Fix: keep the modal open until the guide flow completes, or upload before enabling. **Closed 2026-09-25:** the settings modal stays open until the guide flow completes, so a guide failure shows inline and can be retried.
- **M28. Project docs list shows "Loading…" forever on any non-404 error.** `project/[projectSlug]/pageClient.tsx:276, 316`. Fix: clear loading and set an error on `!res.ok`. **Closed 2026-09-25:** an error state with "Try again".
...
- **M30. `AccountMenu` force-refetches `/api/orgs` on every window focus.** `src/components/AccountMenu.tsx:354-402`. Effect keyed on `session?.user` (new object per session refetch) with `force: true`. Fix: key on email, drop force. **Closed 2026-09-25:** keyed on the user's email, forced only when nothing is cached.
- **M31. "Import from a link" leaves an orphan "Untitled document" on every failed import.** `src/app/HomeAuthedClient.tsx:171-188`. Fix: delete the doc on failure or create it after fetch. **Closed 2026-09-25** on the client: the import flow deletes the document it created on any failure after creation. Server half, same day: `abandonUpload` soft-deletes a document that never had a completed version when its import fails, so a tab closed mid-fetch leaves nothing either. Verified against the dev database for a first version (removed) and a replacement (restored).
- **M32. Components declared inside render remount on every parent render.** `src/app/dashboard/SubscriptionCard.tsx:163-196` (`PlanPanel` → `SpendLimitModule` loses edit state), `src/app/dashboard/page.tsx:316-402` (`DashboardMenu`). Fix: hoist to module scope. **Closed 2026-09-25:** both hoisted, `DashboardMenu` with props.
- **M33. "All pages" mode cancels every in-flight page render on each scroll tick.** `src/components/PdfJsViewer.tsx:1601-1649, 1665-1771`. Fix: only cancel tasks outside the new `toRender` set; drop `pageNumber` from deps. **Closed 2026-09-25:** only renders outside the new set are cancelled, a page already rendering at the same key finishes, `pageNumber` left the deps; a separate effect cancels everything on view-mode or document change.
- **M34. Forced sidebar refresh can coalesce onto a stale in-flight request, resurrecting a just-deleted doc.** `src/lib/sidebarCache.ts:434-437, 477-481`. Fix: when `force` and in-flight, chain a second fetch. **Closed 2026-09-25:** a forced call during an in-flight request queues one follow-up fetch shared by every forced caller meanwhile. `tests/lib/sidebarCacheForce.test.ts`.
...
**Status 2026-09-25: worked through in six passes, one per area, each item re-verified on the current code first.** Closed: all eleven public-share items (timing-safe cookie compare in one helper, fixed error messages, dedupe by document, room emails on the room URL, generic `og:image` when withheld, no writes on anonymous GET, sniffed `page-image` type, projected `og.png`, `React.cache` resolvers, ETag on room thumbnails); all seven docs/uploads items (workspace-scoped lookups, blob allowlist on the two hand-outs, read-only `GET .../changes`, single-link lookups, `buildDocMatch` in thirteen doc routes); all ten projects/notifications items (recomputed `docCount`, `$and` tenancy, hex-only `botIdHash`, `_id` tiebreak, one aggregate for tag counts, one `viewEmailMode` default, request-repo pointers cleared, live-only unique names via migration 0003, fixed messages, no `uploadPath` for viewers); billing/admin/infra except `select: false` on the share password fields (fail-open risk, needs an audit of every `ensureDefaultLink` caller first) and the legacy pay-as-you-go flip (by design); all six MCP/AI items (idempotency per credential, lenient AI schemas, 30-day AiRun TTL via migration 0004, single link read, bounded fan-out) except the slug scan, which needs a by-slug project endpoint; and every frontend item, including 1,093 lines of unreachable viewer-drawer code, the dead `src/app/project` copy, and `/preferences` now redirecting to the dashboard. Still to convert to `buildDocMatch`: sixteen files outside `docs/**` listed in the docs/uploads pass (several are list filters and may be right as written).
