...
- ~~Verified access for sensitive links~~ — **moved to Pro and respecced 2026-09-18 in [lnkdrp-link-access](./lnkdrp-link-access.md).** Three corrections to what this bullet said: access belongs on `ShareLink`, not `Doc` (multi-links shipped after this was written, and two recipients of one deck already have different rights); `password`/`allowlist`/`verified_email` are not one ladder, because an allow list without verification is unenforceable and a password composes with both; and no `lnkdrp_set_share_access` tool is needed, since `lnkdrp_update_share_link` already carries the link's permissions. What stays Enterprise here is **SSO only** — see the next bullet — so the pricing card's "Verified access for sensitive links" should read "Single sign-on for sensitive links", with verified access named on the Pro tier.
