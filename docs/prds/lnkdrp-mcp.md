...
6. **Only the summary is included at launch** (`INCLUDED_ACTIONS_AT_LAUNCH = {summary}` in `src/lib/credits/schedule.ts`; AI compare costs 2/5/12 by tier; doc review has no entry point). Links, settings and stats are free. `share_pdf` accepts an agent-supplied `summary` + `keyPoints`; when present the server stores them and skips its own summary run. The credits preflight stays in the code path (cost 0 today) so the first metered feature from [lnkdrp-credit-features](./lnkdrp-credit-features.md) slots in without a protocol change.
...
**`lnkdrp_whoami`** — In `{}`. Out `{userId, email, orgId, orgName, isPersonalOrg, role, scope, keyPrefix, keyExpiresAt, plan, creditsRemaining, blocked, costs:{summary:[0,0,0], history:[2,5,12]}}` (costs reflect `creditsForRun` live; empty of paid rows until a metered feature ships). Any scope; `readOnlyHint`. Errors `unauthorized`, `key_revoked`.
