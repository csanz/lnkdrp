...
> **Shipped 2026-09-13** for `lnkdrp_share_pdf` and `lnkdrp_set_share_access` (`docs/MCP.md` written; `tests/mcp/e2e.ts` covers the five tools, the 401 at `initialize`, and idempotent replay). `share_pdf` goes through `POST /api/docs` → `/api/uploads` → `import-url` → `process` and waits for `ready` over the realtime channel (self-signed ticket) with 2s polling as fallback; `replaceUrl` is `null` (no capability URLs from MCP). The agent-supplied `summary`/`keyPoints` inputs shipped the same day (40-600 characters, 2-7 key points of at most 160; `POST /api/uploads` validates them, processing skips the AI summary and writes a 0-credit `source: "agent"` ledger row), and `share_pdf`/`get_share` return `warnings` for skipped AI steps. Idempotency is an in-memory `${orgId}:${idempotencyKey}` cache (1000 entries, 24h), not `McpIdempotency`. `lnkdrp_create_request_repo` is **deferred** with the other request tools; `scripts/mcp-smoke.ts` is superseded by the e2e harness.
