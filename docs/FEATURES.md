...
- **On the MCP.** `lnkdrp_get_share_stats { includeVisits: true, visitsLimit? }` adds `recentVisits[]` — the same cards for the document or one link, newest first, with the brief text and viewer identity wrapped as untrusted content. Absent (not `[]`) on Free or when not asked for. `lnkdrp_get_activity` carries the `share.visit_briefed` row and `lnkdrp_whoami.costs.brief` the price.
