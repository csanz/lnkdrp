...
- **What it does.** Posts five moments into a Slack channel the workspace chooses: a recipient's first open of a link, the visit brief after they finish reading, a replaced document (with the change summary), a file received in a request inbox, and (since 2026-09-25) a document added to a project, so a routed room's channel hears what landed. Outbound only: no slash commands, no replies, no bot listening.
