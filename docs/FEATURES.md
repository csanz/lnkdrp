...
- **What it does.** Posts four moments into a Slack channel the workspace chooses: a recipient's first open of a link, the visit brief after they finish reading, a replaced document (with the change summary), and a file received in a request inbox. Outbound only: no slash commands, no replies, no bot listening.
...
- **Per channel.** Four switches (Opens, Visit briefs, Replaced documents, Received files) and a **Projects** section: route a room or a request inbox to the channel from a picker, or take it off with the chip. One project posts to one channel; picking it on another card moves it. A document in two routed rooms posts to both; anything not routed posts to the default. A workspace with a single channel behaves as if the feature had one.
