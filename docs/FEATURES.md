...
- **Dashboard page**: `/dashboard`
  ...
  - **Who pays for AI, and when it is skipped** (`src/app/api/uploads/[uploadId]/process/route.ts`):
    ...
    - **Starter credits**: personal Free workspaces get 50 to start, once — no monthly top-up since 2026-09-16 (both the dashboard snapshot and the reserve path seed through `starterCreditsForWorkspace`; team and Pro workspaces get 0). Once spent, a personal Free workspace can turn on-demand on itself via a pay-as-you-go subscription (`src/lib/billing/subscriptionState.ts`), not only by upgrading to Pro. Free workspaces also have a **15 credits/day** brake (`dailyCreditCap`); hitting it returns `code: "DAILY_CREDIT_CAP"` (402) and the modal says the credits are safe and to try tomorrow. `scripts/credit-balances-reconcile.ts` fixes rows seeded before these rules.
...
- **Scales to hundreds of links per document without rendering them all** (decided 2026-09-16, since
  ...
  `DocLinksManager`'s full table (`variant="page"`) pages through it with Prev/Next; the side-panel
  ...
  by recency, deduped, with `label`/`isDefault` attached via lookup — what `DocQuickStats`'s "Top
...
- **Credits (billing-cycle-based)**:
  ...
  - Personal Free workspaces get **50 credits to start, once** (no monthly top-up since 2026-09-16), with at most 15 credits a day; team workspaces on Free get no allowance. Once spent, personal Free can turn on-demand on itself (pay-as-you-go) at $0.10/credit, same as Pro's on-demand.
...
- **On by default.** Every workspace member has `OrgMembership.viewEmailMode` = `off` | `daily` | `immediate`, default **`daily`**. Existing members read the default (a missing value is `daily`), so nobody has to opt in; a member who sets `off` stays off (no deploy, migration or plan change re-enables it). Views that happen while a member is `off` are dropped, not held: their cursor moves to the current time on every tick, so switching back on only covers views from then on. The mode is per member, per workspace, and independent of the two older preferences.
- **Who gets it**: every member of the workspace that owns the document, each by their own mode, not only the uploader. The cron scans every live membership (unsorted, capped by `limitMembers`, default 500); above that many memberships some members are not reached on a tick, for all three email kinds.
...
- **Immediate**: sent by the `notification-emails` cron (every 5 minutes), so "immediate" means within about five minutes, never in the view request itself. One email per member per document per tick, listing every new viewer on that document since the member's cursor (at most 20 events per member per tick; the rest follow on the next tick). Returning readers are never sent immediately; an immediate member gets them in a returns-only digest on the end-of-day UTC tick, sent only when someone came back that day. Switching from `daily` to `immediate` catches up: the cursor sits at the member's last digest (or where it was created), so the first immediate tick emails every new viewer since then, capped at 7 days, one email per document.
- **Daily digest**: one email per member per UTC day, sent on the end-of-day UTC tick (23:00 UTC onwards, guarded by `lastDigestDay` so overlapping runs send it once), across all of the member's documents: new viewers **and returns**, grouped by document then link. A return is a new `ShareVisit` whose reader (`shareId` + `botIdHash`) already had a `ShareView` at or before the start of the member's window; a reader who first opens and comes back inside the same digest window counts once, as a new viewer. The window runs from the cursor (the latest event in the previous digest) to the tick, so it mostly covers the same UTC day. At most 20 events per digest; the rest go in the next day's. Digest days are UTC for every workspace; `docUpdateDigestTimezone` is not used.
- **Cursor**: `NotificationEmailCursor` key `share_views`, one per member. `lastNotifiedAt` is compared with `ShareView.createdDate`; returns have their own horizon, `returnsNotifiedAt`, compared with `ShareVisit.createdDate`, which only a digest moves (a cursor without it falls back to `lastNotifiedAt`). A missing cursor is created at the current time and nothing is sent (no backfill), so views before a member's first tick are never emailed. The 7-day default lookback (`defaultLookbackDays`) only applies to an existing cursor older than that: the window starts at the later of the cursor and 7 days ago, so events further back behind a stuck cursor are lost. A member on `off` has the cursor moved to now every tick. A failed send is logged: an immediate round stops at the first failed document and the cursor moves to just before that document's earliest event, so it resumes from there on the next tick (documents already sent in that round may repeat); a failed daily digest leaves the cursor where it was and retries on the remaining 23:00–23:59 UTC ticks, then on the next day's 23:00 run.
