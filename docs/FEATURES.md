...
- **Free**: 3 active share links (docs with sharing on, not deleted/archived) — archiving a document frees its slot and its link stops resolving; un-archiving a shared document re-checks the cap, 1 project (request repos do not count), viewer analytics for the last 7 days, no collaborators (just the owner). AI summary, password protection, download control, and MCP/API/CLI access are included. **No version history or AI compare**: Free workspaces get a one-time 50-credit starter grant (`FREE_STARTER_CREDITS = 50`) for upcoming credit features, but no compare, replacement uploads still record a version row but skip the AI compare, the owner history page and "Rerun compare" are gated, and recipients of a Free owner's link get the same "revision history disabled" response as when the toggle is off.
...
- **Upsells on Free** — one registry, two modes. Pro workspaces see none of it (the provider refuses to open the modal for Pro and the modal closes itself if the snapshot resolves to Pro); every surface renders nothing until the plan snapshot has loaded, so there is no layout jump.
  - **Registry** — `UPSELL_COPY` in `src/lib/client/upsellCopy.ts`, keyed `version_history | active_links | projects | collaborators | analytics_history | credits`; each entry has a title, a one-sentence reason and three Pro bullets (the first is always the thing the user was trying to do). Facts mirror `/pricing` (Free: 3 links, 1 project, 7-day analytics, single user; Pro: unlimited links/projects, full analytics history, version history + AI compare, 300 credits a cycle, 1 collaborator included, agents never take a seat). `upsellKeyForLimit` maps a `402` `limit` to a key.
  - **Modal (blocking moments)** — `UpgradeModal`: "Pro" pill, title, reason (+ "{used} of {max} used." and the grace hint when known), three check-marked bullets, "Pro is {price} per workspace. Cancel anytime." (`proPriceLabel` from `GET /api/billing/status`, fetched once per session and cached; `$29/mo` fallback), **Upgrade to Pro** (starts Stripe Checkout for signed-in workspaces via `startCheckout`, otherwise links to `/pricing`) and **Not now**; Escape and the backdrop close it; opening records `markPlanLimitHit`. Opened by: the doc share toggle refused with `402` (`active_links`, switch stays off); the revision-history toggle refused with `402` and the clickable **Pro** pill next to it (`version_history`); the history page's **See what's included** button (the page keeps a one-line "Version history is a Pro feature" note so it is not empty); the Teams invite note's **Upgrade** button (`collaborators`; the form stays disabled; Pro with its included collaborator in place still gets the "includes one collaborator · Contact us" note); the New-project modal's inline note (at the cap the note shows immediately and **Create project** is disabled; its **Upgrade to Pro** closes the create modal, then opens the upgrade modal with `projects`); and the sidebar meter's **Upgrade to Pro** (`active_links` or `projects`, whichever is at limit).
  - **Inline (passive states)** — kept quiet and in place; their Upgrade links open the modal for the matching key instead of navigating to `/pricing`: the left-sidebar block above the account menu ("Links x of 3" / "Projects x of 1" thin bars, `src/components/PlanUsageMeter.tsx`; amber "At your link limit" at the cap); the upload-page note at the link cap ("This workspace is at its 3-link limit. You can still upload; sharing stays off until you free a link or upgrade." — the upload is never blocked); the doc page's "Turning this on needs a free link slot (3 of 3 used)" hint under the share switch; the "Free shows the last 7 days · Upgrade for full history" footnotes on the metrics page and in `DocQuickStats` (`analytics_history`); the dashboard Plan card's Free meters with **See what's included** (Pro shows "Unlimited links · Unlimited projects · Full history · 1 collaborator included"); and the sidebar's fallback `PlanLimitNotice` nudge. The small **Pro** pill (`src/components/ProPill.tsx`) marks version/history links on Free.
