...
- **Harness**: `npx tsx --env-file=.env.local tests/mcp/e2e.ts` mints a temporary key, connects as `lnkdrp-e2e`, runs the fourteen tools plus the 401, idempotent-replay and destructive-confirmation checks, and revokes the key.
