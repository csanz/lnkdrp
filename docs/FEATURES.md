...
- **Dashboard page**: `/dashboard`
  ...
  - Overview includes a **Plan** card at the top:
    ...
    - Free plan shows **live meters** from `GET /api/plan` (Docs x of 10 · Projects x of 2 · Analytics "Basic · 7 days" · Members 1 of 1; the documents bar turns amber at the cap) with a **Compare plans** link, plus a **single** **Upgrade** CTA (Stripe Checkout via `POST /api/stripe/checkout`) and a **View plan details** link to `/pricing` (the old in-dashboard plan modal is gone; `/pricing` is the single source of truth for plan comparison). Pro shows "Unlimited links · Unlimited projects · Deep analytics · 3 collaborators included".
...
- **Pro** (per workspace): unlimited links and projects, **deep analytics** with full history (viewer names/emails, per-viewer views/time/pages/downloads, time per page, return visits, visit timelines), 3 collaborators included, **a version list recipients can browse** (`shareAllowRevisionHistory`; owner version history and AI compare at 2/5/12 credits by tier are on every plan), **500 credits a month (reset on the Stripe renewal date, no rollover)** and **on-demand credits at $0.10 each** once a spend limit is set. **Paid seats are deferred**: extra members will be announced (and priced) before they are billed; agents never count as seats.
...
- **A document's home, and staying inside it** (`docs/prds/lnkdrp-project-home.md`, built 2026-09-25):
  - **Upload into a data room.** The upload page has an "Add to a data room" picker (preselected by `/upload?project=<id>`; a project page has an "Upload here" button that opens it), `POST /api/docs { projectId }` and `lnkdrp_share_pdf { projectId | projectSlug }` create the document inside the room from its first write (`projectIds` + `primaryProjectId`), so there is one creation event: the feed row reads "… in Data room", Slack's "was added to Data room" post goes to the room's channel, and no separate "added to project" row follows. A request inbox is refused (400 `PROJECT_IS_INBOX`).
  - **Contained documents.** `Doc.visibility: "workspace" | "project"`. A contained document is listed only inside its primary project: not in the Docs sidebar, `/api/docs`, search, tag pages, starred, the recent-changes feed, the dashboard, workspace metrics, the suggested-docs list or `lnkdrp_list_docs`; one shared filter (`src/lib/docs/visibility.ts`, `workspaceListableDocFilter`) is spread into every workspace-wide listing and `tests/lib/containedDocListings.test.ts` pins each one. Its rows leave the workspace activity feed and show under `/api/activity?projectId=`. Slack routes it to the room's channel or nowhere, never to the catch-all. Its direct URL, metrics, history and links keep working for the workspace. Set at upload ("Only inside this data room"), from the document menu ("Keep inside its data room" / "List in the workspace"), `PATCH /api/docs/:id { visibility }` and `lnkdrp_set_doc_visibility`; a contained document refuses a second project (409 `CONTAINED`). The document page shows a **Contained** pill; the feed records `doc.contained` / `doc.uncontained`.
...
- **Billing → Funnel**: `/a/funnel` — the upgrade funnel per ISO week (UTC), newest first, for the last 8, 12 or 26 weeks: workspaces created, workspaces hitting their first ever plan limit and any limit, shown the upgrade or out-of-credits prompt, pressing Upgrade / Pack / Compare / Dismiss on it, shown the Free analytics teaser (with the median number of readers it was showing, from `funnel.teaser_shown`), starting a Pro or credit-pack Checkout, and upgraded by the webhook. Above the table: the median days from workspace creation to the first wall and which limit is the first wall (the Phase 5 decisions in the pricing plan). Every figure counts distinct workspaces. `GET /api/admin/funnel?weeks=N`; the reads are `type` + `createdDate` bounded (`src/lib/funnel/report.ts`), served by the activity `type` index rather than a scan of the feed.
...
- **Contributor pages**: `/people/:userId` and `/agents/:client/:ownerUserId` — one contributor, and everything they have done in this workspace. Reached from the bold subject name on any feed row, which is now a link. The pages are built out of the feed's own parts (`src/components/activity/ActivityRows.tsx`, `src/components/activity/useActivityPages.ts`) rather than beside them: the same app-shell header, the same rows, the same **All / Uploads / Sharing / Documents / Projects / Views / Members** tabs and the same Previous/Next + page-size rail, so a contributor page is a *view of the feed* and cannot drift from it. People and agents share one component (`src/components/people/ActorPageClient.tsx`) because they are the same page: a person shows an email and the agents they connected, an agent shows a "Connected by …" line back to its owner (a tombstoned owner reads "a member who has left"; no owner at all, "an unknown member"), and everything below means the same for both.
  - **Above the feed**: all-time tiles in the `/activity` stats-header shape — first activity, last activity, Actions, the five `ACTIVITY_SUMMARY_BUCKETS`, and **Other work** (the remainder, so the tiles never add up to less than Actions with nothing to explain the gap) — and a right rail of **Agents connected** (person only), **Documents** and **Projects**, each line naming the thing and how many actions went into it. A deleted document keeps its rail row, struck through and unlinked: the work happened, and dropping it would make the totals disagree with the list.
  - **Data**: the profile is `GET /api/activity/actor?key=<contributor key>`; the feed underneath is the ordinary `/api/activity` narrowed with `actor=<key>`, live arrivals included. The endpoint answers 404 for a key with no activity in the active workspace, so a foreign id cannot be used to probe. Both routes are `force-dynamic` and validate the id with `keyFromRoute` before rendering, so a hand-typed or truncated id shows the not-found UI (streamed, so with HTTP 200).
- **A row belongs to its agent, not to the member who connected it.** The subject link takes the first of the reader's page, the row's agent, then the row's actor (`subjectHrefFor` in `ActivityRows.tsx`), so a row an agent wrote opens `/agents/claude-code/<owner>` even when the sentence prints the member's name (`doc.replaced` does). The link's tooltip names the destination in that case — "See everything Claude Code changed" — so a person's name never silently opens an agent's page. Agents are keyed per owner (`agent:<client>@<ownerUserId>`, `src/lib/people/contributorKey.ts`): two members who each connect Claude Code are two contributors with two pages, and `unknown` is a real owner segment (rows from a credential whose creator was never recorded) with a page but no person to link back to.
...
- **Funnel rows** (`docs/reviews/pricing-upsell-fix-plan-2026-09-23.md`, Phase 4.1): `plan.limit_reached` is written by `planLimitResponse` itself when the route passes who hit the wall (`{ limit, used, max, grace }`, once per workspace and limit per 10 minutes per instance), so the feature gates count too; `funnel.modal_shown` / `funnel.cta_clicked` come from the browser through `POST /api/funnel` (`{ reason, cta: upgrade | pack | compare | manage | dismiss, from }`, `from` being the route shape the modal opened on, e.g. `doc.x.metrics`); `checkout.started` is written by `POST /api/stripe/checkout` (`{ kind: "pro", interval }`) and `POST /api/credits/purchase` (`{ kind: "credit_pack", pack, credits, priceCents }`). The feed hides the funnel rows, Checkout starts and feature-gate refusals unless a `type=` filter names them (`src/lib/activity/feedVisibility.ts`); the counted-cap refusals stay visible.
...
The entry under **Activity** in the left sidebar (`/contacts`). Decided in `docs/prds/lnkdrp-contacts.md`, built 2026-09-25. Every plan has it; who a contact is follows the plan exactly as the analytics do.

- **What a contact is.** One row per `(orgId, email)` in `Contact` (`src/lib/models/Contact.ts`), the same key `ShareViewerEmail` uses, so a person who reads four decks is one row with four documents on it. It carries the latest name and the first one given, the domain (null for the webmail providers, so `gmail.com` never reads as a company), first and last seen, the sources they arrived through (capped at 50), the documents and projects touched, a visit counter, and the team's one note. `verifiedAt` is never copied: the page reads it from `ShareViewerEmail` by the shared key, so a confirmation cannot be true in one table and false in the other.
- **Nobody types one in.** `upsertContact` (`src/lib/contacts/service.ts`) runs at the four moments the product already records a person: an introduction on a document link or in a data room (landing and stats routes), a signed-in read, a download request, and an upload to a request inbox that asks people to sign in (a public inbox collects no address, so it makes no contact). One idempotent `findOneAndUpdate` each, from `after()`, never throwing into the path that called it, and never for the owner's own preview. A signed-in read counts once a sitting, not once a heartbeat. A reader who gives no address stays a visit, not a person.
- **Identity follows the plan.** On Free a contact who never introduced themselves comes back with name and email `null` (the row, the domain and the dates stay), because introductions are shown on Free everywhere else. Pro shows everyone in full. The redaction happens in the service, so the page, the CSV and the MCP tools cannot disagree, and every response carries `identity: true|false` so a client explains the blanks rather than drawing an empty column.
- **The list.** `/contacts`: search (name, address or domain, escaped regex), sort by last seen, first seen, name, domain, documents read or visits in either direction, and filters by tag, by how they arrived and by domain. Every control is mirrored into the URL, paging is 50 a page, and the Free banner above the table says what is being held back. `GET /api/contacts` takes the same words.
- **Download CSV.** `GET /api/contacts/export` takes the list's filters and sort and answers `text/csv` as `contacts-<workspace>-<yyyymmdd>.csv`, RFC 4180 quoted, with a UTF-8 BOM so Excel reads an accented name as one. Streamed from an async generator in batches of 500, so nothing holds the list in memory and the browser starts writing at once. `countContacts` runs first: past `CONTACTS_CSV_MAX_ROWS` (25,000) the route answers 413 with the count, and the page's button says "Too many to download" rather than offering a link that fails, because a download is a 200 the moment it starts and cannot be told later that it is short. Redacted rows have empty name and email cells, never a placeholder.
- **The contact page.** `/contacts/:id`: who they are (with a **Verified** pill when the address is confirmed), the note, the tags, then the history: the documents they read (each linking to that document's metrics), the rooms they entered, and every arrival. `PATCH /api/contacts/:id` writes the note; member role or above, never an API key, 2,000 characters, and it records `contact.note_updated` in the feed.
- **Tags on a person.** `TAG_TARGET_KINDS` gained `"contact"`, so the picker, the assignment routes and the tag page are the tag system's own. The tag page lists contacts in a third section beside documents and projects. Writing a tag on a person is a person's call: no agent tool does it.
- **Agents.** `lnkdrp_list_contacts` and `lnkdrp_get_contact`, read-only, in `mcp/src/tools/contacts.ts`; names, addresses, domains and the note are wrapped as untrusted content. See `docs/MCP.md`.
- **Elsewhere.** Account purge deletes the workspace's contacts (`purgeCompleteness` pins it). Deleting a workspace soft-deletes it the way it soft-deletes its documents and memberships, so the contacts stay unreachable behind the dead memberships until the account purge erases them, exactly as `ShareView` and `ShareViewerEmail` do. The backfill migration `db/migration/20260925_0006_contacts_backfill.mjs` builds the table from `ShareView`, `ProjectLinkView`, `ShareDownloadRequest`, `ShareViewerEmail` and the `request.upload_received` rows with a signed-in uploader, so the page is full on the day it ships. Help: `/help/contacts`.
...
- **Identity follows the analytics tier** (the email is never a side channel around the Pro gate):
  ...
  - **Free**: says *that* a recipient opened *which link* and when; no name, pages or time. Carries a secondary button, **See who opened it**, to the document's metrics page (where the real viewer count teaser lives) when the primary button went to the reader page, else to `/pricing`; either way with `from=view_email`. The line "Pro shows who opened it and how long they stayed" is the button's caption.
...
- **The reader is on the row.** A data-room reader introduces themselves once on the landing page, and that lands on the link's `ShareView` rows, not on the brief row (the viewer's timing posts carry no name). When the visit is written up, `pickReaderIdentity` fills the row's missing name, email or account id from the newest share view that has it, so the stored brief, its email, its Slack post and the reader page all name the same person. Found on the Slack routing test, 2026-09-25.
...
- **On the reader's page.** `/doc/:id/metrics/viewer/:key` and the project twin show a **Visit briefs** section above the sessions: one card per finished sitting (`briefed`, `recap`, `failed`; skipped visits have none), newest first, with the date, duration, pages, downloads *during that visit*, which visit number it was, and the brief — headline, body, "What caught their attention", highlights, next step. A recap card says why there is no write-up and offers **Write the brief · 1 credit** (`POST /api/visits/:visitBriefId/brief`, `src/lib/visits/visitBriefs.ts` `writeVisitBriefNow`): the same reserve/generate/charge/store as the cron, billed to the member who clicked, past the automatic-briefs and per-day gates on purpose (those protect against spend without a click), with the usual `402 OUT_OF_CREDITS` / `DAILY_CREDIT_CAP` and the out-of-credits modal. The row is claimed first so a double click or a concurrent cron cannot write it twice; no second email is sent, but the feed gets its `share.visit_briefed` row with `meta.source: "manual"`. Data: `GET /api/docs/:docId/visit-briefs` and `GET /api/projects/:projectId/visit-briefs` (`?kind=anon&botIdHash=` / `?kind=authed&userId=`, `?shareId=`), Pro only (`402`); `VisitBriefCards` renders nothing on Free.
- **On the MCP.** `lnkdrp_get_share_stats { includeVisits: true, visitsLimit? }` adds `recentVisits[]` — the same cards for the document or one link, newest first, with the brief text and viewer identity wrapped as untrusted content. Absent (not `[]`) on Free or when not asked for. `lnkdrp_get_activity` carries the `share.visit_briefed` row and `lnkdrp_whoami.costs.brief.credits` the price.
- **Downloads belong to the sitting, not the day.** The three download routes push the instant onto `ShareView.downloadedAt` (last 50) beside the by-day count, and the snapshot counts the instants inside the sitting's window plus the quiet window's slack ("read it, clicked download, closed the tab"). Rows without instants fall back to the day, which is how two same-day sittings once both said "then downloaded the deck".
- **Faster than the tick.** The realtime server (`realtime/server.ts`, `briefPoker`) already sees every reader write on the `shareviews` stream; it keeps one timer per (workspace, link, reader), and when a reader has been quiet for the window plus 15 s it `POST`s `/api/cron/visit-briefs?workspaceId=` with the cron secret. The route still decides everything (due rows only, postpones a returning reader, lease against overlap); this is only "look now", so a closed tab is briefed in about 2–3 minutes and the five-minute cron remains the backstop. Needs `REALTIME_APP_URL` (or the site-URL variables) and, in production, `CRON_SECRET`; without them it logs once and stays off. Pinned to the engine's window by `tests/lib/visitBriefAccelerator.test.ts`.
- **Not yet.** The cron does detection, generation and sending today; the plan is to move the body to a queue and worker, which is why `runVisitBriefs`/`settleVisitBrief`/`writeVisitBriefNow` are plain functions.

## Integrations (Slack)

The first entry under **Integrations** in the left sidebar (`/integrations`, one card per integration; only Slack today). Decided in `docs/prds/lnkdrp-slack.md`, built 2026-09-24. Available on every plan; what a message may say follows the plan like the emails do.

- **Gated by scale, not as a feature** (2026-09-26). Free connects `FREE_SLACK_CHANNELS` (1) channel and it posts all five kinds of message; the second channel and project routing are Pro (`slack_channels` as a counted cap, `slack_routing` as a feature gate). The cap is enforced in the OAuth callback rather than at the button, because the install is Slack's own screen and the callback is reachable without passing the button: past the cap it stores nothing and comes back as `?slack=error&reason=plan_limit`. Reconnecting a channel the workspace already has is an update, never a new channel, so it is always allowed, and a channel Slack revoked does not count against the cap. `PATCH /api/orgs/active/slack` refuses a non-empty `projectIds` on Free with the usual 402 and allows an empty one, so a workspace that drops to Free can undo its own routing. The page shows a Pro pill on **Add channel** and on the routing picker rather than offering a link that would fail at Slack's screen. The reasoning is in `FREE_SLACK_CHANNELS`: a Free user watching the posts land every day is the upsell, and a locked page is not.

- **What it does.** Posts five kinds of moment into a Slack channel the workspace chooses: a recipient's first open of a link (and their introduction), the visit brief after they finish reading, a replaced document with what changed or a new share link, a file received in a request inbox, and a new document (finished uploading, or added to a data room). Outbound only: no slash commands, no replies, no bot listening.
- **Connecting.** `/integrations/slack`, "Add to Slack" (owners and admins). The install is Slack's own screen with the `incoming-webhook` scope, where the person picks the channel (private channels work). The callback exchanges the code, stores the webhook URL encrypted (AES-256-GCM under a key derived from `LNKDRP_SLACK_SECRET`, falling back to `NEXTAUTH_SECRET`), and the first channel becomes the default. "Add channel" runs the install again for another channel. "Send a test message" and "Disconnect" on each card; a channel Slack removed shows as **Disconnected** with the reason. Needs `SLACK_CLIENT_ID` and `SLACK_CLIENT_SECRET`; without them the page says Slack is not set up on this deployment and the install route answers 503.
- **Per channel.** Five switches (Opens, Visit briefs, Replaced documents and links, Received files, New documents) and a **Projects** section: route a room or a request inbox to the channel from a picker, or take it off with the chip. One project posts to one channel; picking it on another card moves it. A document in two routed rooms posts to both; anything not routed posts to the default. A workspace with a single channel behaves as if the feature had one.
- **How a post gets there.** One `SlackOutbox` row per channel per event, keyed so a re-fired event posts once, written at the event and posted from `after()` at that moment. A failure leaves the row pending with the email queue's backoff (1 m, 5 m, 30 m, 2 h, 12 h; dead after five) and the `notification-emails` cron drains it; the `visit-briefs` cron drains after its own send. At most 30 posts a minute per channel; the rest wait a minute and the channel gets one "and N more" line. A webhook Slack reports gone marks the connection revoked and skips its batch. Sent rows expire after 30 days.
- **What a message says.** Block Kit with a plain-text twin: one line, one context line, one link back to the document, the reader's page or the history. The blocks sit in an attachment with a coloured left bar, which is what gives the channel a hierarchy: the accent green for anything a recipient did (opened, introduced themselves, finished reading, sent a file to a request inbox), grey for what the workspace did to its own documents (added, replaced, a new link). Each kind leads with one emoji (👀 open, 👋 introduction, 📖 brief, 📨 received, 📄 added, 🔄 replaced, 🔗 new link), in the blocks only: the plain-text twin stays clean because Slack reads it alone for mobile notifications and screen readers. A project is marked wherever it is named — 📁 for a data room, 📥 for a request inbox, the one distinction the app itself draws (`isRequest`); projects carry no icon of their own anywhere in the product, and `roomEmoji` in `src/lib/slack/messages.ts` is the single place that would change if they ever did. Link previews are refused on every post (`unfurl_links` and `unfurl_media` both false), since every URL points into the signed-in app. Names and titles are escaped for Slack markup and never turned into mentions. Reader identity follows the plan exactly as in the emails: Pro shows the name or email, Free shows "Someone". A data-room reader who introduced themselves on the landing page is named on every later open and brief: the renderer and the brief row read the link's arrival and share-view rows (`src/lib/share/readerIdentity.ts`), the same two collections the activity feed reads. The introduction itself posts as its own line ("Elena Ruiz introduced themselves on Data room via Sequoia Capital"), on Free too, as in the feed. Visit-brief posts carry the headline and body when there is one, or the plain facts for a recap. The four document changes each have their own line: "Q3 Deck was added" (with the page count, linking to the document and its metrics), "Q3 Deck was replaced, now v3" (with the change summary, linking to the history), "Q3 Deck was added to Acme data room" (linking to the room), and "New link Investors for Q3 Deck" (with the link's audience, linking to the document's links or the room). None of them names a reader, so they read the same on every plan.
- **Elsewhere.** Activity rows `integration.slack_connected` and `integration.slack_disconnected` (filter group Integrations). `lnkdrp_whoami.integrations.slack` reads the channels and the mapping back for agents; nothing writes it. Account purge removes connections and outbox rows.
