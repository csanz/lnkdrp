...
- **Elsewhere.** Account purge and workspace deletion delete the workspace's contacts (`purgeCompleteness` pins it). The backfill migration `db/migration/20260925_0006_contacts_backfill.mjs` builds the table from `ShareView`, `ProjectLinkView`, `ShareDownloadRequest` and `ShareViewerEmail`, so the page is full on the day it ships. Help: `/help/contacts`.
