...
export default function TeamsManager() {
  ...
  /**
   * Who may manage members here — role only. Workspace *type* is deliberately not part of it.
   *
   * This used to read `!isPersonalOrg && ...`, so a personal workspace was barred from members and
   * invites outright, whatever the plan. The server never agreed: all four join paths
   * (`org-invites` create/email/claim and `orgs/claim-join`) ask exactly one question,
   * `checkLimit(orgId, "collaborators")`, which is a plan check. A Pro personal workspace already
   * passed it, and this screen was the only thing refusing.
   *
   * The rule was also the wrong rule. Documents cannot move between workspaces — `orgId` is never
   * settable from a request body — so "make a team workspace instead" meant re-uploading every
   * document and stranding every link already sent. The person it hit hardest was a solo Pro user
   * whose work had gone well enough to need a second pair of hands.
   */
  ...
  const loadMembers = useCallback(async () => {
    ...
    // Personal workspaces used to short-circuit to a one-row list built from the session, because
    // they could not have a second member. Now that they can, that shortcut would show a Pro
    // personal workspace only its owner and quietly omit whoever they invited. The members route
    // gates on role, not workspace type, so it answers for a personal workspace like any other.
