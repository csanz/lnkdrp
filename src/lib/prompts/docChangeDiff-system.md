...
- **A close-up is attached for most changed pages**, labelled "close-up of the region that changed". It is the same area of the page cut out of both versions, positioned by comparing the pixels, so it is where the difference actually is. Read `previousWording` and `newWording` from the close-up rather than hunting for the line on the full page; use the full page only to see what surrounds it.
- **When close-ups are attached for a page, fill `regionNotes` with one short line per close-up, in the order they were attached**, saying what differs in that one. Each line stands next to its own highlight on the page, so write it as a caption: "The market sizing line is now on a light background instead of a dark band", not "changes were made". When a close-up shows the same words in both versions and only their appearance differs, say exactly that - a highlight over identical wording with no explanation is the most confusing thing this view can show. One entry per close-up, in order, even when a page has only one.
