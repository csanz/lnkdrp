...
export const TOOL_CATALOG: ToolCatalogEntry[] = [
  ...
  },
  {
    ...
    access: "read",
    detail: {
      inputs: ["docId and/or shareId: a shareId scopes everything to that one link; a docId covers all its links", "days: 1 to 60, default 15; Free is clamped to 7", "includeViewers: per-viewer rows with per-page time (Pro only)"],
      output: "Totals for views, opens, downloads, pages viewed and time spent, a unique viewer count, and a per-day series. On Pro with includeViewers it also returns every reader, with their pages and the time spent on each page. Owner and teammate opens are excluded from every figure and counted separately as ownerPreviews.",
  ...
  },
  {
    ...
    access: "write",
    detail: {
      inputs: ["docId", "label: private name, 1 to 80 characters, never shown to viewers", "audience: optional private note, up to 120 characters", "allowDownload / allowRevisionHistory: optional", "password: optional, 1 to 128 characters", "expiresAt: optional ISO date in the future", "enabled: optional, default on"],
