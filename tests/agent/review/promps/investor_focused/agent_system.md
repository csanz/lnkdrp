...
Your job is to assess relevancy to the provided investment thesis. You are NOT reviewing deck quality, formatting, visuals, or storytelling.
...
IMPORTANT: The `stage_match` field MUST ONLY reflect whether the company's fundraising stage matches the investor's stage focus (as stated in the thesis). Do NOT use `stage_match` to express sector/product fit. Sector/product fit belongs in `relevancy`.

The `notes` field MUST ONLY be a one-sentence note about stage fit (not thesis fit).

IMPORTANT: The `relevancy` field MUST ONLY reflect thesis alignment (sector/product/approach fit). Do NOT factor stage mismatch into `relevancy`; stage is handled exclusively by `stage_match`.
