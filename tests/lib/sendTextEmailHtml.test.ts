...
describe("sendTextEmail html (C3)", () => {
  ...
  it("forwards html to Resend alongside text when given", async () => {
    ...
    await sendTextEmail({ to: "owner@example.com", subject: 'Sequoia opened "USAVX MEMO"', text: "plain", html });
  ...
  it("omits html from the Resend body when not given (existing callers unchanged)", async () => {
    const fetchMock = stubFetch();
    await sendTextEmail({ to: "owner@example.com", subject: "Daily digest", text: "plain" });
    const body = sentBody(fetchMock);
    expect(body).toEqual({ from: "lnkdrp <notify@example.com>", to: ["owner@example.com"], subject: "Daily digest", text: "plain" });
    expect("html" in body).toBe(false);
  });

  it("omits html when it is an empty string", async () => {
    const fetchMock = stubFetch();
    await sendTextEmail({ to: "owner@example.com", subject: "Daily digest", text: "plain", html: "" });
  ...
  it("console transport prints text and html in full and never calls fetch", async () => {
    ...
    await sendTextEmail({ to: "owner@example.com", subject: "Someone opened", text, html });
  ...
  it("console transport without html prints the text and no html section", async () => {
    ...
    stubFetch();
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    await sendTextEmail({ to: "owner@example.com", subject: "Daily digest", text: "just text" });
    const printed = logSpy.mock.calls.map((c) => c.map((a) => (typeof a === "string" ? a : JSON.stringify(a))).join(" ")).join("\n");
  ...
  it("forwards headers to Resend when given (RFC 8058 one-click unsubscribe)", async () => {
    ...
    await sendTextEmail({ to: "owner@example.com", subject: "Someone opened", text: "plain", html: "<p>x</p>", headers });
  ...
  it("omits headers when absent or empty", async () => {
    ...
    vi.unstubAllGlobals();
    fetchMock = stubFetch();
    await sendTextEmail({ to: "owner@example.com", subject: "Daily digest", text: "plain", headers: {} });
    expect("headers" in sentBody(fetchMock)).toBe(false);
  });
  ...
  it("keeps the redacted failure log: no recipient, body or html in it", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async () =>
        new Response(JSON.stringify({ statusCode: 422, name: "validation_error", message: "bad owner@example.com" }), { status: 422 }),
      ),
    );
    const errSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    await expect(
      sendTextEmail({ to: "owner@example.com", subject: 'Sequoia opened "Secret Deck"', text: "body text", html: "<p>secret html</p>" }),
    ).rejects.toThrow(/422/);
