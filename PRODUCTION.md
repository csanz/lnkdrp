...
| Realtime (WebSockets) | **Deployed 2026-09-24.** Fly app `lnkdrp-realtime` (org `lnkdrp`, iad, one machine) on the read-only user `lnkdrp_prod_realtime` against `lnkdrp-prod`; `https://realtime.lnkdrp.com/healthz` answers `ok`, Mongo connected, all seven streams up, Let's Encrypt certificate issued, CNAME `realtime → lnkdrp-realtime.fly.dev`. `NEXT_PUBLIC_REALTIME_URL=wss://realtime.lnkdrp.com` set on Vercel Production and the web app rebuilt. Not yet: `REALTIME_APP_URL` + `CRON_SECRET` on the Fly app, so the visit-brief accelerator is off and briefs wait for the five-minute cron. Dev twins `dev-lnkdrp-realtime` / `dev-lnkdrp-mcp` also exist on Fly. | *verified*, curl + fly |
| Slack integration | **Not configured.** `SLACK_CLIENT_ID`, `SLACK_CLIENT_SECRET` and `LNKDRP_SLACK_SECRET` are not set on Vercel, so `/integrations/slack` shows "not set up on this deployment" and the install route answers 503. The code (connect, four events, project routing, whoami read-back) is on `next-release`, not yet on `main`. Set-up steps in `DEPLOY.md` 4.7; needs a Slack app created at api.slack.com first. | *verified* env names |
