...
| `REALTIME_DISABLED` | leave unset. Set to `1` to turn realtime off at request time without a rebuild — the kill switch in 10, Kill switches. `NEXT_PUBLIC_REALTIME_URL` is inlined at build time, so it is not one |
...
| Realtime | Set `REALTIME_DISABLED=1` and redeploy. `/api/realtime/ticket` reads it at request time and answers the empty payload clients already treat as "poll instead", so open tabs stop retrying as soon as the env var lands. Unsetting `NEXT_PUBLIC_REALTIME_URL` also works but is slower: it is inlined at build time, so it needs a **fresh build**, and until that lands every open tab keeps hammering a dead server and waking this route (dynamic, two Mongo reads) every 30 s per tab. The MCP server has its own copy (`deploy/fly/mcp.fly.toml`) and polls by itself when the socket fails | Next deploy |
